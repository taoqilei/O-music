...
public class LrcView extends View {
    ...
    private long mNextTime = 01; //保存下一句开始的时间
    private int mViewWidth; // view的宽度
    private int mLrcHeight;// lrc 界面的高度
    ...
    protected  void onMeasure(int widthMeasureSpec, int heightMeasureSpec){
        ...
        //重新设置view的高度
        int measureHeightSpec = MeasureSpec.makeMeasureSpec(mLrcHeight, MeasureSpec.AT_MOST);
    ...
    protected  void onDraw(Canvas canvas){
        if (mBackground != null){
            canvas.drawBitmap(Bitmap.createScaledBitmap(mBackground,mViewWidth,mLrcHeight,true), new Matrix(), null);
        }

        float centerY = (getMeasuredHeight() + mTextBounds.height() -mDividerHeight)/2;
        if (mLrcs.isEmpty() || mTimes.isEmpty()){
            canvas.drawText(DEFAULT_TEXT,(mViewWidth - mCurrentPaint.measureText(DEFAULT_TEXT))/2, centerY, mCurrentPaint);
            return;
        }
        ...
        for (int i = mCurrentLine-1,j=1;i >= firstLine;i--,j++){
            ...
            canvas.drawText(lrc, x, centerY - j * offsetY - offsetY, mNormalPaint);
        }
